...
from parallel_stride import StrideReshapeLayer2D,\
    FinalMeanLayer
...
def residual_block(l, 
    nonlinearity=lasagne.nonlinearities.rectify, survival_prob=1,
    batch_norm_alpha=0.1, batch_norm_epsilon=0.01,
    increase_units_factor=None, half_dims=False, projection=False,
    ):
    ...
    if half_dims:
        stack_1 = StrideReshapeLayer2D(stack_1,stride=(2,2))
    ...
    if half_dims:
        ...
        shortcut = StrideReshapeLayer2D(shortcut, stride=(2,2))
    if increase_units_factor is not None:
        if projection:
...
def build_cnn(input_var=None, n=5):
    ...
    l = batch_norm(ConvLayer(l_in, num_filters=16, filter_size=(3,3), 
    ...
    l = residual_block(l, increase_units_factor=2, half_dims=True)
    for _ in range(1,n):
        l = residual_block(l)
    ...
    # new part, replacing global pooling + dense by 1x1 conv and final meaner
    # and nonlinearity layer
    l = lasagne.layers.Conv2DLayer(l, num_filters=10, filter_size=1,
                                     nonlinearity=lasagne.nonlinearities.identity)
    
    l = FinalMeanLayer(l)
    l = lasagne.layers.NonlinearityLayer(l,
